...
def create_nuscenes_infos(root_path,
    ...
    # 1/4 sample of data
    train_scenes_subset = random.sample(train_scenes, k=int(0.25 * len(train_scenes)))
    val_scenes_subset = random.sample(val_scenes, k=int(0.25 * len(val_scenes)))

    test = 'test' in version
    if test:
        print('test scene: {}'.format(len(train_scenes_subset)))
    else:
        # print('a')
        print('train scene: {}, val scene: {}'.format(
            len(train_scenes_subset), len(val_scenes_subset)))

    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, nusc_map_extractor, nusc_can_bus, train_scenes_subset, val_scenes_subset, test, max_sweeps=max_sweeps)
    ...
    if test:
        ...
        info_path = osp.join(out_path,
                             '{}_infos_test_1/4.pkl'.format(info_prefix))
        ...
        data = dict(infos=train_nusc_infos, metadata=metadata)
        info_path = osp.join(out_path,
                             '{}_infos_train_1/4.pkl'.format(info_prefix))
        mmcv.dump(data, info_path)
        data['infos'] = val_nusc_infos
        info_val_path = osp.join(out_path,
                                 '{}_infos_val_1/4.pkl'.format(info_prefix))

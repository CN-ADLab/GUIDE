...
class InstanceBank(nn.Module):
    def __init__(
        ...
        gs_offset = (torch.rand(self.per_num_gs, 3, dtype=torch.float) - 0.5) * 2
        gs_offset = self.scale_range[0] + (self.scale_range[1] - self.scale_range[0]) * gs_offset
            
        gs_scale = torch.rand_like(gs_offset)
        gs_scale = safe_inverse_sigmoid(gs_scale)

        gs_rots = torch.zeros(self.per_num_gs, 4, dtype=torch.float)
        gs_rots[:, 0] = 1
        
        # gs_opacity = safe_inverse_sigmoid(0.1 * torch.ones((self.per_num_gs, 1), dtype=torch.float))
    
        gs_anchor = torch.cat([gs_offset, gs_scale, gs_rots], dim=-1)

        self.gs_anchor = nn.Parameter(
            torch.tensor(gs_anchor, dtype=torch.float32),
            requires_grad=gs_anchor_grad,
        )
        self.gs_anchor_init = gs_anchor
        self.gs_feature = nn.Parameter(
            torch.zeros([per_num_gs, self.embed_dims]),
            requires_grad=gs_feat_grad,
        )
    ...
    def init_weight(self):
        ...
        self.gs_anchor.data = self.gs_anchor.data.new_tensor(self.gs_anchor_init)
        if self.gs_feature.requires_grad:
            torch.nn.init.xavier_uniform_(self.gs_feature.data, gain=1)
    ...
    def get(self, batch_size, metas=None, dn_metas=None):
        ...
        gs_feature = torch.tile(self.gs_feature[None], (batch_size, 1, 1))
        gs_anchor = torch.tile(self.gs_anchor[None], (batch_size, 1, 1))
